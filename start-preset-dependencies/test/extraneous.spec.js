  ...
  it('should list dependencies present in managed*Dependencies, but not in modules', done => {
    ...
    project.within(() => {
      return start(extraneous())
        ...
        .catch(e => {
          expect(reporter).to.have.been.calledWith(sinon.match.any, 'info', 'Extraneous managedDependencies: adash, highdash');
          expect(reporter).to.have.been.calledWith(sinon.match.any, 'info', 'Extraneous managedPeerDependencies: bar');
